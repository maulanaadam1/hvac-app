    ...
    try {
      ...
      if (res.ok) {
        ...
        data.forEach((p: any) => {
          if (!grouped[p.module]) {
            grouped[p.module] = [];
            expanded[p.module] = true; // default expanded
          }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col h-full pb-12">
      ...
      <div className="flex flex-col space-y-4 mb-6">
        <div className="text-xs font-medium text-gray-500 flex items-center gap-1.5">
          ...
          <span className="text-gray-800">Create Role</span>
        </div>
        
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-gray-800">Create Role</h2>
          <p className="text-sm text-gray-500 mt-1">
            Create a new role and configure its permissions and scope.
          </p>
        </div>
      </div>
          ...
          <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6">
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-start gap-2.5 cursor-pointer">
                ...
                <div>
                  <div className="text-sm font-bold text-gray-800">Custom Scope</div>
                  <div className="text-[10px] text-gray-500">Define custom scope (sites, buildings, floors)</div>
                </div>
              </label>
            </div>
          </div>
